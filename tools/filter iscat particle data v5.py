...
''' COLORS '''
#GeoDataViz 0.9 Diverging bold
colors = ['#045275','#089099','#7ccba2','#fcde9c','#f0746e','#dc3977','#7c1d6f']
#GeoDataViz - mdk edit, 9 colors
#colors = ['#045275','#089099','#7ccba2','#b7e6a5','#fce1a4', '#f0746e','#dc3977','#7c1d6f']
#GeoDataViz 0.9 Diverging bold - mdk edit - 5 colors
colors = ['#089099','#7ccba2','#fcde9c','#f0746e','#dc3977']#,'#7c1d6f']
#'#045275',
trendline_color = '#f0746e'
#GeoDataViz (chat gpt edit) - 10 colors
colors = ['#024c66', '#045275', '#089099', '#7ccba2', '#fcde9c', '#f49b78', '#f0746e', 
          '#dc3977', '#7c1d6f', '#5a0e52']
...
def remove_clusters2(pl_in, space_eps, time_eps):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(scriptpath, "FILTERED de-cluster map.png"))
    #plt.show()
    
    pl2 = list(np.array(pl_in)[kept_indices])
...
def remove_long_range_moving_particles(pl_in, max_dist=50):
    ...
    y = pdist_
    labels = pIDs_
    fig, ax = plt.subplots(nrows=2, ncols=2, figsize=(18, 18), dpi=300)
    ...
    for p in pl_in:
        ax[0, 1].plot(p.px_vec, p.py_vec, color=matplotlib.cm.viridis(p.pID/len(pl_in)))
    ...
    #draw particle position traces
    for p in pl_out:
        ax[1, 1].plot(p.px_vec, p.py_vec, color=matplotlib.cm.viridis(p.pID/len(pl_in)))
    
    
    scriptpath, scriptfilename = os.path.split(__file__)
    plt.savefig(os.path.join(scriptpath, "FILTERED long range particle IDs" + TAG + ".png"))
    #plt.show()
...
''' PLOT DATA '''
def plot_two_pls_with_mass_split(pl, pl2, volttime, voltdata, mass_split):
...
def plot_kinetics(pl, mass_split, tag):
...
def export_data(pl_in, pltag, savevid=True):
    save_pl(pl_in, pltag)
    plot_map(pl_in, pltag)
    plot_contrast(pl_in, 0, pltag)
    plot_kinetics(pl_in, 0, pltag)
    if savevid==True: save_video_from_pl(pl_in, r8, pltag)
    np.savetxt(os.path.join(scriptpath, "particle count"+pltag+" -- "+str(len(pl_in))), [len(pl_in)])
    
#filter particles by lifetime
pl1 = filter_pl_by_lifetime(pl0, lifetime=int(0.25*constants["bufsize"]))
export_data(pl1, "-pl1")

#filter data by confidence
pl2 = filter_pl_by_conf(pl1, cavg=0, chigh=0.20) 
export_data(pl2, "-pl2")

#filter particles if there are many breaks
pl3 = [p for p in pl2 if (p.f_vec[-1]-p.f_vec[0]) - len(p.f_vec) < 10] #filter "flashing" particles (ones that are recorded with many skipped frames )
export_data(pl3, "-pl3")

#remove long range particles 
pl4 = remove_long_range_moving_particles(pl3, max_dist=50)
export_data(pl4, "-pl4")

#remove repeated clusters of particles
pl5 = remove_clusters2(pl4, space_eps=5, time_eps=2) #i was originally using 5 and 10
export_data(pl5, "-pl5")
...
# #split masses and generate results
# mass_split = 0#0.007
# #print plot and save output
# plot_two_pls_with_mass_split(pl, pl4, volttime, voltdata, mass_split=mass_split)
# plot_contrast(pl4, mass_split)
# plot_kinetics(pl4, mass_split)
# generate_landing_rate_csv(pl4, "", mass_split)


# #%%
# #make new video
# r8 = load_video_cv2()
# save_video_from_pl(pl4, r8, "r4")
# #%%

# for p in pl3[:630]:
#     print(f"{p.pID} \t {len(p.f_vec)} \t {(p.f_vec[-1]-p.f_vec[0])/len(p.f_vec)} \t {(p.f_vec[-1]-p.f_vec[0]) - len(p.f_vec)}")
