...
def plot_map(pl, tag):
    ...
    #sm = plt.cm.ScalarMappable(cmap=plt.cm.viridis_r, norm=plt.Normalize(vmin=0, vmax=np.max(contrasts)))
    #!!!
    #!!!
    #!!!
    
    for p in pl:
        plt.scatter(np.mean(p.px_vec), np.mean(p.py_vec), color=matplotlib.cm.viridis_r(p.std_max/max_std))
...
def plot_contrast_kinetics(pl_in, tag):
    ...
    ax2.plot(volttime, voltdata, color=matplotlib.cm.viridis(0.9))
    vymax = 2
    if np.max(voltdata) >= 2: vymax = np.max(voltdata)+0.1
    ax2.set_ylim(-0.2, vymax)
    ax2.set_ylabel("volts")
...
def get_brightdata():
    ...
    #print("SCRIPTPATH:", scriptpath)
    #timestamp = os.path.split(scriptpath)[1][:19]
    brightfileloc = scriptpath #os.path.dirname(scriptpath)
    brightfilename = "screen brightness.csv"
    brightfilepath = os.path.join(brightfileloc, brightfilename)
    #print("VOLT FILE PATH:", voltfilepath, "\n")
    #print("TIMESTAMP:", timestamp)
    #print("VOLT FILE NAME:", voltfilename, "\n")
    brightdata = np.loadtxt(brightfilepath, delimiter=",", usecols=0)
    return brightdata



def plot_contrast_kinetics_vs_brightness(pl_in, tag):
    #!!!
    #!!!
    #!!!
    fig, ax = plt.subplots(figsize=(10,8), dpi=300)
    ...
    ''' BRIGHTNESS DATA '''
    brightdata = get_brightdata()
    brighttime = np.linspace(0, len(brightdata)/constants["fps"], len(brightdata))
    ax2.plot(brighttime, brightdata, color=matplotlib.cm.viridis(0.3))
    #vymax = 2
    #if np.max(voltdata) >= 2: vymax = np.max(voltdata)+0.1
    #ax2.set_ylim(-0.2, vymax)
    ...
    ''' SAVE '''
    scriptpath, scriptfilename = os.path.split(__file__)
    plt.savefig(os.path.join(scriptpath, "FILTERED contrast brightness kinetics" + tag + ".png"))
    ...
    #plot the ratio of contrast to background darkness
    plt.clf()
    
    # normalize contrast to screenbrightness
    from scipy.interpolate import interp1d
    print(len(avg_time), len(avg_contrast), len(brightdata))
    y = avg_contrast
    x_original = np.linspace(0, 1, len(avg_contrast))
    x_new = np.linspace(0,1, len(brightdata))
    interp_func = interp1d(x_original, y, kind='linear')
    y_interp = interp_func(x_new)
    print(len(x_new), len(y_interp))
    plt.scatter(brighttime, y_interp*brightdata)
    plt.savefig(os.path.join(scriptpath, "FILTERED contrast x brightness relationship" + tag + ".png"))
    plt.clf()
    return
...
def export_data(pl_in, pltag, save_vid=False):
    ...
    plot_contrast_kinetics_vs_brightness(pl_in, pltag)
    save_pl(pl_in, pltag)
